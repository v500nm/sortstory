      ...
      <div className="flex flex-col md:flex-row w-full flex-1 overflow-hidden">
        ...
        <aside className="w-full md:w-72 bg-brand-bg-medium border-b md:border-b-0 md:border-r border-brand-border flex flex-col shrink-0">
          ...
          <div className="p-3 md:p-5 border-b border-brand-border bg-brand-bg-medium z-10">
            <nav aria-label="Sidebar Breadcrumb" className="mb-2">
              <ol className="flex items-center flex-wrap gap-1 text-[11px] font-mono text-brand-text-secondary">
                <li>
                  <Link href="/" className="hover:text-brand-purple transition-colors flex items-center gap-1">
                    <span>🏠</span> Home
                  </Link>
                </li>
                <li className="text-brand-border-light">/</li>
                <li>
                  <Link href="/learn" className="hover:text-brand-purple transition-colors">
                    Learn
                  </Link>
                </li>
                <li className="text-brand-border-light">/</li>
                <li>
                  <Link href={`/learn#${topic}`} className="hover:text-brand-cyan transition-colors capitalize font-semibold text-brand-cyan/90">
                    {formattedTopic}
                  </Link>
                </li>
              </ol>
        ...
        <main className="flex-1 overflow-y-auto bg-brand-bg-dark">
          <div className="max-w-4xl mx-auto px-4 py-6 sm:p-8 lg:p-12">
            ...
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-4 border-b border-brand-border gap-4">
              <nav aria-label="Content Breadcrumb" className="flex items-center flex-wrap gap-1.5 text-xs font-mono">
                <Link href="/" className="text-brand-text-secondary hover:text-brand-purple transition-colors">
                  Home
                </Link>
                <span className="text-brand-border-light">/</span>
                <Link href="/learn" className="text-brand-text-secondary hover:text-brand-purple transition-colors">
                  Learn
                </Link>
                <span className="text-brand-border-light">/</span>
                <Link href={`/learn#${topic}`} className="text-brand-cyan/80 hover:text-brand-cyan transition-colors capitalize font-semibold">
                  {formattedTopic}
                </Link>
                <span className="text-brand-border-light">/</span>
                <span className="text-brand-purple font-bold capitalize">
                  {formattedAlgo}
                </span>
                <span className="text-brand-border-light">/</span>
