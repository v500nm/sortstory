...
void heapSort(int arr[], int n) {
    ...
    "shellSort": {
      "name": "Shell Sort",
      "best": "O(N log N)", "avg": "O(N^(1.5))", "worst": "O(N²)", "space": "O(1)",
      "desc": "Shell Sort is an in-place comparison sort. It is a generalization of insertion sort that allows the exchange of far apart elements.",
      "flow": [
  ...
  return arr;
    ...
    for (int gap = n / 2; gap > 0; gap /= 2) {
      ...
      "q1": "Which gap sequence is typically used in Shell's original version of Shell Sort?\n- [x] n/2, n/4, ..., 1\n- [ ] Knuth's sequence (3^k - 1) / 2\n- [ ] Sedgewick's sequence\n- [ ] Prime numbers",
      "q2": "Why is Shell Sort faster than standard Insertion Sort on average?",
      "q3": "Write a Python script executing Shell Sort using Knuth's sequence."
    },
    "radixSort": {
      "name": "Radix Sort",
      "best": "O(d * (N + b))", "avg": "O(d * (N + b))", "worst": "O(d * (N + b))", "space": "O(N + b)",
      "desc": "Radix Sort is a non-comparative sorting algorithm. It avoids comparison by creating and distributing elements into buckets based on radix of individual digits.",
      "flow": [
...
void radixSort(int arr[], int n) {
    ...
    for (int exp = 1; m / exp > 0; exp *= 10) countSort(arr, n, exp);
      ...
      "q1": "Which stable sorting algorithm is typically used as a subroutine within Radix Sort?\n- [ ] Quick Sort\n- [x] Counting Sort\n- [ ] Selection Sort\n- [ ] Heap Sort",
      "q2": "What are the limitations of Radix Sort regarding the format of sorting keys?",
      "q3": "Write a Python implementation of Radix Sort handling negative numbers."
    }
  }
};

function generateAllDsaCurriculum() {
  const mdDir = path.join(process.cwd(), 'md_files');

  Object.entries(curriculum).forEach(([category, data]) => {
...
# ${details.name} (GeeksforGeeks Reference)
...
`;
  ...
  console.log("GFG-style DSA curriculum created successfully!");
}

generateAllDsaCurriculum();
